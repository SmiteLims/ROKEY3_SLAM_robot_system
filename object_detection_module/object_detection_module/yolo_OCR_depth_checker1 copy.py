...
TARGET_CLASS_ID = 0  # YOLO에서 검출할 대상 클래스 ID
DEPTH_SCALE = 1.0    # 깊이 값 보정 스케일
...
class YoloOCRDepthChecker(Node):
    def __init__(self):
        ...
        # 퍼블리셔
        self.image_publisher_ = self.create_publisher(CompressedImage, '/robot1/yolo_ocr/image/compressed', qos_profile)
        self.vehicle_info_publisher_ = self.create_publisher(VehicleInfo, 'vehicle_info', qos_profile_vehicle)
        self.distance_publisher_ = self.create_publisher(Int32, '/robot1/yolo_ocr/near', qos_profile)

        # 구독자
        self.create_subscription(PoseWithCovarianceStamped, '/robot1/amcl_pose', self.amcl_pose_callback, qos_profile2)
        ...
        self.create_subscription(CompressedImage, RGB_TOPIC, self.rgb_callback, 1)
        self.create_subscription(CompressedImage, DEPTH_TOPIC, self.depth_callback, 1)
    ...
    def depth_callback(self, msg):
        with self.lock:
            self.depth_image = self.bridge.compressed_imgmsg_to_cv2(msg, 'passthrough')
    ...
    def processing_loop(self):
        while rclpy.ok():
            ...
            for i in range(len(tracked)):
                ...
                roi_size = 5
                half = roi_size // 2
                x_start = max(0, cx_depth - half)
                x_end = min(depth_w, cx_depth + half + 1)
                y_start = max(0, cy_depth - half)
                y_end = min(depth_h, cy_depth + half + 1)
                ...
                dist_m = center_depth / 1000.0 if depth.dtype == np.uint16 else float(center_depth)
                dist_m *= DEPTH_SCALE
                ...
                if not self.ocr_done:
                    ...
                    for text_bbox in ocr_result:
                        ...
                        if len(text) == 4 and text.isdigit():
                            ...
                            vehicle_info_msg.location = "Location Test!"
                            self.vehicle_info_publisher_.publish(vehicle_info_msg)
                            self.get_logger().info(f"[VehicleInfo] 차량 번호: {text} - 위치: {vehicle_info_msg.location} 퍼블리시 완료")
                            self.ocr_done = True
                            break
            ...
            # ✅ CompressedImage 메시지로 퍼블리시
            ros_image = self.bridge.cv2_to_compressed_imgmsg(annotated)
